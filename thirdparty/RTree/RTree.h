...
template <class DATATYPE, class ELEMTYPE, 
          class ELEMTYPEREAL = ELEMTYPE, int TMAXNODES = 30,
...
protected:
  ...
  bool Subsumes(Rect *a_rectA, Rect *a_rectB) const;
...
RTREE_TEMPLATE
bool RTREE_QUAL::Subsumes(Rect *a_rectA, Rect *a_rectB) const {
  ASSERT(a_rectA && a_rectB);
...
bool RTREE_QUAL::Subsumes(Node *a_node, Rect *a_rect) const {
  ...
  if (a_node->IsInternalNode()) {
    // This is an internal node in the tree
    for (int index = 0; index < a_node->m_count; ++index) {
      if (Subsumes(a_node->m_branch[index].m_child, a_rect))
        return true;
    }
  } else {
    // This is a leaf node
